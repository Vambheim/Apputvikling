...
const ShoppingCartCard = props => {
  ...
  const onAddPress = () => {
    const newPrice = props.price + props.originalPrice;
    ...
    <Pressable onPress={onCardPress}>
      <View
        ...
        <View>
          ...
          <Text style={{color: '#F4F2F3', fontFamily: 'Avenir'}}>
            {i18n.t('Amount')}: {incrementCount}
          </Text>
        </View>
        <View style={{flexDirection: 'row'}}>
          <Pressable onPress={onAddPress} style={styles.button}>
            <Text style={{color: '#F4F2F3'}}>+</Text>
          </Pressable>
          <Pressable onPress={onSubtractPress} style={styles.button}>
            <Text style={{color: '#F4F2F3'}}>-</Text>
          </Pressable>
        </View>
      </View>
